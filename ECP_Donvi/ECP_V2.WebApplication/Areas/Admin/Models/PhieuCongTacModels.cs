using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace ECP_V2.WebApplication.Areas.Admin.Models
{
    public class PhieuCongTacModels
    {
        public int phieucongtacid { get; set; }
        public int phienlvid { get; set; }
        public string phieucongtac { get; set; }
        public string LanhDaoCongViec { get; set; }
        public string LanhDaoCongViec_Id { get; set; }
        public string NguoiChiHuy { get; set; }
        public string NguoiChiHuy_Id { get; set; }
        public string GiamSatVien { get; set; }
        public string GiamSatVien_Id { get; set; }
        public string NguoiCapPhieu { get; set; }
        public string NguoiCapPhieu_Id { get; set; }
        public string NguoiKiemSoat { get; set; }
        public string NguoiKiemSoat_Id { get; set; }
        public string NguoiKiemTraPhieu { get; set; }
        public string NguoiKiemTraPhieu_Id { get; set; }
        public string PhongBanID { get; set; }
        public string NoiDung { get; set; }
        public string DiaDiem { get; set; }
        public string GioBd { get; set; }
        public string GioKt { get; set; }
        public string DieuKienAnToan { get; set; }
        public string DonViQLVHLienQuan { get; set; }
        public string NoiDatTai { get; set; }
        public string AnToanKhac { get; set; }
        public string NhanVienDonViCongTac_SelectEmp { get; set; }
        public string NgayGioKT_B2 { get; set; }
        public string NgayGioKT_B3 { get; set; }
        public string DonViLienQuanQLVH { get; set; }
        public string ChiTietCatDien { get; set; }
        public string ChiTietNoiDat { get; set; }
        public string ChiTietRaoChan { get; set; }
        public string ChiTietBienBao { get; set; }
        public string PhamViLamViec { get; set; }
        public string CanhBaoNguyHiem { get; set; }
        public string SoNguoiThamGia { get; set; }
        public string PhongBanID_PCT { get; set; }
        public string PhongBan_PCT { get; set; }
        public string LoaiCongViec { get; set; }
        public string MaYeuCauCRM { get; set; }
        public int? TramId { get; set; }

    }


}