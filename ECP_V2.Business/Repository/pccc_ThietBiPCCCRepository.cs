...
{
    public class pccc_ThietBiPCCCRepository : RepositoryBase<pccc_ThietBiPCCC>
    {
        public string Connectstr { get; set; }
        public pccc_ThietBiPCCCRepository(string connect)
            : base()
        {
            this.Connectstr = connect;
        }

        public pccc_ThietBiPCCCRepository(WorkUnit unit)
            : base(unit)
        {
        }
        ...
        public override string Delete(object entityId, ref string strError)
        {
            try
            {
                ...
                var entity = Context.pccc_ThietBiPCCC.SingleOrDefault(o => o.ID == id);
                Context.pccc_ThietBiPCCC.Remove(entity);
                Context.SaveChanges();
                strError = "";
                return "success";
            }
        ...
        public override object Create(pccc_ThietBiPCCC entity, ref string strError)
        {
            try
            {
        ...
        public override pccc_ThietBiPCCC GetById(object entityId)
        {
            try
            {
                var id = int.Parse(entityId.ToString());
                return Context.pccc_ThietBiPCCC.SingleOrDefault(p => p.ID == id);
            }
            catch { return null; }
        }

        public override List<pccc_ThietBiPCCC> List()
        {
            try
            {
                return Context.pccc_ThietBiPCCC.ToList();
            }
            catch (Exception ex) { return null; }
        }

        public override List<pccc_ThietBiPCCC> ListByQuery(string strQuery)
        {
            throw new NotImplementedException();
        }

        public override object Update(pccc_ThietBiPCCC entity, ref string strError)
        {
            try
            {
        ...
        public override List<pccc_ThietBiPCCC> GetTableListById(object entityId)
        {
            throw new NotImplementedException();
        }
        ...
        public int CountByMaHieu(string MaHieu)
        {
            try
            {
                ...
                {
                    string query = "select count(ID) from pccc_ThietBiPCCC where MaNhom=1 and MaHieu='" + MaHieu;
        ...
        public int CountByMaHieu_Edit(string MaHieu, string ID)
        {
            try
            {
                using (IDbConnection db = new SqlConnection(Connectstr))
                {
                    string query = "select count(ID) from pccc_ThietBiPCCC where MaNhom=1 and MaHieu='" + MaHieu + " and ID!= " + ID;
                    var q = db.Query<int>(query);
                    return q.FirstOrDefault();
                }
            }
        ...
        public List<pccc_ThietBiPCCCModel> ListPaging(int page, int pageSize, string filter, string TuNgay, string DenNgay,
            ...
            )
        {
            ...
            */
            ...
            List<pccc_ThietBiPCCCModel> lstData = new List<pccc_ThietBiPCCCModel>();
            try
            {
                ...
                using (SqlConnection db = new SqlConnection(Connectstr))
                {
                    string query =
                        ...
                        ",NgayKiemTra=(select top(1) NgayKiemTra from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc) " +
                        ",GhiChu=(select top(1) GhiChu from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc) " +
                        ",NgayKiemTraTiepTheo= (select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc) " +
                        ",TenTT=(select Name from pccc_TrangThai where ID=tb.MaTT) " +
                        ",TrangThai= " +
                        "( " +
                        "case when " +
                        "(DATEDIFF(d, (select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB = tb.ID order by ID desc), getdate()) > 0) " +
                        "then 1 " +
                        "else " +
                        "(case when(DATEDIFF(d,getdate(),(select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc)) <=15 " +
                        "and DATEDIFF(d,getdate(),(select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc)) >=0)  " +
                        "then 2 " +
                        "else " +
                        "(case when(DATEDIFF(d,getdate(),(select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc)) <=30 " +
                        "and DATEDIFF(d,getdate(),(select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc)) >15)  " +
                        "then 3 " +
                        "else " +
                        "(case when DATEDIFF(d,getdate(),(select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc)) >30 " +
                        "then 4 else 5 " +
                    ...
                    {
                        query = query + "and ( " +
                            "DATEDIFF(d,getdate(),(select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc)) <=15 " +
                            "and DATEDIFF(d,getdate(),(select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc)) >=0 " +
                            ") "
                            ;
                    }
                    ...
                    {
                        query = query + "and ( " +
                            "DATEDIFF(d,getdate(),(select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc)) <=30 " +
                            "and DATEDIFF(d,getdate(),(select top(1) NgayKiemTraTiepTheo from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc)) >15 " +
                            ") "
                            ;
                    }
                    ...
                    {
                        query = query + "and ( " +
                            "(select top(1) isnull(KetQua,0) from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc) =1 " +
                            ") "
                            ;
                    }
                    ...
                    {
                        query = query + "and ( " +
                            "(select top(1) isnull(KetQua,0) from pccc_SoTheoDoiPCCC where MaTB=tb.ID order by ID desc) =0 " +
                            ") "
                            ;
                    }
                    //ko co kiem dinh
                    ...
                    {
                        query = query + "and ( " +
                            "(select count(ID) from pccc_SoTheoDoiPCCC where MaTB=tb.ID) =0 " +
                            ") "
                            ;
                    }
                    ...
                    {
                        var q = multipleresult.Read<pccc_ThietBiPCCCModel>();
                        lstData = q.ToList();
                    }
                }
            }
        ...
        public string DeleteAll(string[] entityId, ref string strError)
        {
            try
            {
                var entity = Context.pccc_ThietBiPCCC.Where(o => entityId.ToList().Contains(o.ID.ToString()));
                Context.pccc_ThietBiPCCC.RemoveRange(entity);
                Context.SaveChanges();
                strError = "";
                return "success";
            }
        ...
        public int Count()
        {
            try
            {
                using (IDbConnection db = new SqlConnection(Connectstr))
                {
                    string query = "select count(tb.ID) from pccc_ThietBiPCCC tb " +
                        "where tb.MaNhom=1 ";
                    var q = db.Query<int>(query);
                    return q.FirstOrDefault();
                }
        ...
        public int CountByMaPhieu(string MaHieu)
        {
            try
            {
                using (IDbConnection db = new SqlConnection(Connectstr))
                {
                    string query = "select count(ID) from pccc_ThietBiPCCC tb " +
        ...
        public List<pccc_ThietBiPCCCModel> Export(string filter, string TuNgay, string DenNgay,
           string DonViId, string PhongBanId, string TrangThai, string MaLoai, string TrangThaiKiemDinh, string MaNhom, string MaTT
           )
        {
            string start1 = "";
    ...
    public class pccc_ThietBiPCCCModel
    {
    ...
    public class pccc_BieuDoTronModel
    {
