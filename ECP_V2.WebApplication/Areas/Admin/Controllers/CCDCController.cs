...
{
    public class CCDCController : UTController
    {
        ...
        private CongCuDungCuAnToanRepository _CCDCAT_ser = new CongCuDungCuAnToanRepository(System.Configuration.ConfigurationManager.ConnectionStrings["IdentityDbContext"].ConnectionString);
        ...
        private SoTheoDoiCCDCRepository _So_ser = new SoTheoDoiCCDCRepository();
        private TaiLieuSoCCDCRepository _TaiLieuSo_ser = new TaiLieuSoCCDCRepository();
        private DonViRepository _dvi_ser = new DonViRepository();
        private PhongBanRepository _phongban_ser = new PhongBanRepository();
        private TrangThaiCCDCRepository _TrangThai_ser = new TrangThaiCCDCRepository(System.Configuration.ConfigurationManager.ConnectionStrings["IdentityDbContext"].ConnectionString);
        ...
        // GET: Admin/CCDC
        [HasCredential(MenuCode = "CCDC;DsCCDC;DsCCLD")]
        ...
        public ActionResult Create(int? MaNhom)
        {
            try
            {
                ...
                CongCuDungCuAnToanViewModels Model = new CongCuDungCuAnToanViewModels();
                Model.KetQua = true;
        ...
        [HttpPost]
        public ActionResult Create(CongCuDungCuAnToanViewModels model, HttpPostedFileBase[] browsefile)
        {
            string kt = "|";
            CreateDropHangSX(null);
            CreateDropNuocSX(null);
            ...
            CreateDropTrangThai(null);
            SoTheoDoiCCDCAT objs = new SoTheoDoiCCDCAT();
            string strError = "";
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!model.switchTTKD.GetValueOrDefault() && model.SoLuong > 0)
                    {
                        ...
                        {
                            ThietBiATLD obj = new ThietBiATLD();
                            ...
                            if (model.switchTTKD.GetValueOrDefault())
                            {
                                ...
                                if (browsefile != null && browsefile.Length > 0)
                                {
                                    foreach (HttpPostedFileBase ad in browsefile)
                                    {
                                        if (ad != null)
                                        {
                                            SoTheoDoiCCDCAT_TaiLieu objtl = new SoTheoDoiCCDCAT_TaiLieu();
                                            ...
                                            if (int.Parse(tl.ToString()) != 0)
                                            {
                                                ...
                                                string path = Server.MapPath("~/DocumentFiles/SoTheoDoiCCDCAT/" + Session["DonViID"].ToString());
                                                if (!Directory.Exists(path))
                                                {
                                                    Directory.CreateDirectory(path);
                                                }
                                                path = System.IO.Path.Combine(Server.MapPath("~/DocumentFiles/SoTheoDoiCCDCAT/" + Session["DonViID"].ToString() + "/"), fileName);
                                                ad.SaveAs(path);
            ...
            string DonViId, string PhongBanId, string TrangThai, string MaLoai, string TrangThaiKiemDinh, string MaNhom, string MaTT)
        {
            ...
            List<CongCuDungCuAnToanModel> model;
            ...
            //var ListNewsPageSize = new PageData<CongCuDungCuAnToanModel>();
            //if (model.Count() > 0)
            //{
            ...
            //else
            //{
            //    ListNewsPageSize.Data = new List<CongCuDungCuAnToanModel>();
            //    ListNewsPageSize.Page = new Page()
            //    {
            //        RecordsName = "Công cụ dụng cụ an toàn",
            //        NumberOfPages = 0,
            ...
            var ListNewsPageSize = new PagingV2.PageData<CongCuDungCuAnToanModel>();
            if (model.Count() > 0)
            {
                ListNewsPageSize.Data = model;
        ...
        {
            CongCuDungCuAnToanViewModels Model = null;
            try
            {
                ...
                Model = new CongCuDungCuAnToanViewModels();
                Model.ID = obj.ID;
        ...
        public ActionResult Edit(CongCuDungCuAnToanViewModels model)
        {
            string kt = "|";
            try
            {
        ...
        public ActionResult CreateKD(int id)
        {
            try
            {
                ...
                Model.ID = id;
                //thong tin kiem tra
                var objso = obj.SoTheoDoiCCDCATs.OrderByDescending(o => o.ID).FirstOrDefault();
                if (objso != null)
                {
        ...
        [HttpPost]
        public ActionResult CreateKD(CongCuDungCuAnToanViewModels model, HttpPostedFileBase[] browsefile)
        {
            string kt = "|";
            CreateDropHangSX(null);
            CreateDropNuocSX(null);
        ...
        {
            List<SoTheoDoiCCDCATViewModels> model = new List<SoTheoDoiCCDCATViewModels>();
            try
            {
                var obj = _CCDCAT_ser.GetById(id);

                //thong tin kiem tra
                foreach (var item in obj.SoTheoDoiCCDCATs)
                {
                    SoTheoDoiCCDCATViewModels objs = new SoTheoDoiCCDCATViewModels();
                    objs.ID = item.ID;
        ...
        public ActionResult DeleteKiemDinh(int id)
        {
            ...
            try
            {
                ...
                var lsttl = obj.SoTheoDoiCCDCAT_TaiLieu.ToList();
        ...
        public bool DeleteFile(string filename)
        {
            try
            {
                string fullPath = Request.MapPath("~/DocumentFiles/SoTheoDoiCCDCAT" + filename);
        ...
        {
            SoTheoDoiCCDCATViewModels Model = null;
            try
            {
                var obj = _So_ser.GetById(id);

                Model = new SoTheoDoiCCDCATViewModels();
                Model.ID = obj.ID;
                ...
                ViewBag.lstTaiLieu = obj.SoTheoDoiCCDCAT_TaiLieu.ToList();
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        [ValidateAntiForgeryToken]
        public ActionResult EditKD(SoTheoDoiCCDCATViewModels model, HttpPostedFileBase[] browsefile)
        {
            string kt = "|";
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    objs.BienBanSo = model.BienBanSo;
                    objs.KetQua = model.KetQua;
                    if (model.NgayKiemTra != null && objs.ThietBiATLD.HanKiemDinh != null)
                        objs.NgayKiemTraTiepTheo = model.NgayKiemTra.Value.AddMonths(objs.ThietBiATLD.HanKiemDinh ?? 0);
                    objs.GhiChu = model.GhiChu;
                    ...
                    else
                    {
                        if (browsefile != null && browsefile.Length > 0 && browsefile[0] != null)
                        {
                            ...
                            foreach (var it in objs.SoTheoDoiCCDCAT_TaiLieu)
                            {
        ...
        public ActionResult ImportExcelCCDC(HttpPostedFileBase file, int hdMaNhom)
        {
            ...
            try
            {
                    ...
                    if (dt != null)
                    {
                        for (int i = 1; i < dt.Rows.Count; i++)
                        {
                            ...
                            try
                            {
                                ...
                                if (PhongBanID != 0)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            strErrorSum.AppendLine("<hr/>" + "(Dòng: " + (i + 1).ToString() + ") Không tìm thấy Loại CCDC: <b>" + dt.Rows[i][1].ToString().Trim() + "</b>");
                                            continue;
                                        }
                                    }
                                    catch (Exception)
                                    {
                                        strErrorSum.AppendLine("<hr/>" + "(Dòng: " + (i + 1).ToString() + ") Loại CCDC gặp lỗi");
                                        continue;
                                    }
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            strErrorSum.AppendLine("<hr/>" + "(Dòng: " + (i + 1).ToString() + ") Tên CCDC không được để trống");
                                            continue;
                                        }
                                    }
                                    catch (Exception)
                                    {
                                        strErrorSum.AppendLine("<hr/>" + "(Dòng: " + (i + 1).ToString() + ") Tên CCDC gặp lỗi");
                                        continue;
                                    }
                                    ...
                                    try
                                    {
                                        if (!string.IsNullOrEmpty(dt.Rows[i][10].ToString()))
                                        {
                                            ...
                                            {
                                                strErrorSum.AppendLine("<hr/>" + "(Dòng: " + (i + 1).ToString() + ") Không tìm thấy Trạng thái CCDC: <b>" + dt.Rows[i][10].ToString().Trim() + "</b>");
                                                continue;
                                            }
                                        }
                                    }
                                    ...
                                    {
                                        strErrorSum.AppendLine("<hr/>" + "(Dòng: " + (i + 1).ToString() + ") Trạng thái CCDC gặp lỗi");
                                        continue;
                                    }
                            ...
                            catch (Exception ex)
                            {
                                ...
                                NLoger.Error("loggerDatabase", string.Format("Lỗi tạo công cụ dụng cụ. Chi tiết: {0}", ex.Message));
                                return RedirectToAction("Index", "CCDC");
                            }
                        }
                ...
                return RedirectToAction("Index", "CCDC", new { MaNhom = hdMaNhom });
            }
            catch (Exception ex)
            {
                DisposeAll();
        ...
        private void ExportExcelFromList(IEnumerable<CongCuDungCuAnToanModel> list, string DateFrom, string DateTo)
        {
            try
            {
                ...
                if (donviId == null)
                {
                    ...
                    rowIndex++;
                    rowTerminal = sheet.CreateRow(rowIndex);
                    rowTerminal.CreateCell(0).SetCellValue("BÁO CÁO CÔNG CỤ DỤNG CỤ");
                    rowTerminal.Cells[0].Row.Height = 350;
                    rowTerminal.Cells[0].CellStyle = styleHeader1;
                ...
                rowTerminal.CreateCell(1).SetCellValue("Tên CCDC an toàn");
                ...
                rowTerminal.CreateCell(7).SetCellValue("Trạng thái CCDC");
                ...
                using (var exportData = new MemoryStream())
                {
                    ...
                    if (donviId == null)
                    {
                        strFileName = string.Format("Bao-cao-cong-cu-dung-cu_{0}.xlsx", DateTime.Now).Replace("/", "-");
                    }
                    else
                    {
                        strFileName = string.Format("Bao-cao-cong-cu-dung-cu_{0}.xlsx", DateTime.Now).Replace("/", "-");
