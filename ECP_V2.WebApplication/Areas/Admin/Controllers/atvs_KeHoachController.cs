...
{
    public class atvs_KeHoachController : Controller
    {
        private DonViRepository _dv_ser = new DonViRepository();
        private atvs_KeHoachRepository _kehoachRepository = new atvs_KeHoachRepository(System.Configuration.ConfigurationManager.ConnectionStrings["IdentityDbContext"].ConnectionString);

        // GET: Admin/atvs_KeHoach
        [HasCredential(MenuCode = "ATVS_KEHOACH")]
        ...
        public ActionResult GetInfo(string donviId, int? year)
        {
            ...
            var model = _kehoachRepository.GetAllByOption(year.Value, donviId);
        ...
        [HttpPost]
        public ActionResult SaveNoiDung(atvs_KeHoach model)
        {
            try
            {
                if (model.Id == 0)
                {
                    ...
                    var newM = _kehoachRepository.Add(model);
                ...
                else
                {
                    ...
                    _kehoachRepository.Update(model);
        ...
        public ActionResult ChuyenNPC(string donviId, int? year)
        {
            try
            {
                ...
                _kehoachRepository.Update_ChuyenNPC(donviId, year.Value, User.Identity.Name);
        ...
        public ActionResult Export(string donviId, int? year)
        {
            ...
            List<atvs_KeHoachViewModel> model = new List<atvs_KeHoachViewModel>();

            model = _kehoachRepository.GetAllByOption(year.Value, donviId);


            string sWebRootFolder = System.Web.Hosting.HostingEnvironment.MapPath("~/Content/MauNhapLieu/ATVS");
            string sFileName = $"BaoCao_ATVSLD_KeHoach_{DateTime.Now:yyyyMMddhhmmss}.xlsx";
            // Template File
            string templateDocument = Path.Combine(sWebRootFolder, "BaoCao_ATVSLD_KeHoach_Template.xlsx");
