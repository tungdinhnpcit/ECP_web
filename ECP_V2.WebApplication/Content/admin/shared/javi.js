var javi = {
    ...
    convertDateJS: function (value) {
        var pattern = /Date\(([^)]+)\)/;
        var datetime = pattern.exec(value);

        if (datetime === null || datetime === '')
            return '';
        var newdate = new Date(parseFloat(datetime[1]));
    ...
    startLoading: function () {
        ...
        if (!$('#page-wrapper').hasClass('sk-loading')) {
    ...
    getTaskStatus: function (status) {
        if (status == 1)
            ...
            return '<span class="badge" style="background-color:#27ae60; color:#fff">Hoàn thành</span>';
        else if (status == 4)
            return '<span class="badge" style="background-color:#1b30eb; color:#fff">Chờ duyệt</span>';
        else if (status == 5)
            return '<span class="badge" style="background-color:#DC0000; color:#fff">Hết hạn</span>';
    },
