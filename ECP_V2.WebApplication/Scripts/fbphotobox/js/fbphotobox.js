(function ($) {
    ...
    FBPhotoBox.prototype = {
        ...
        show: function (image) {
            ...
            $(".fbphotobox-container-left-footer").html('<div style="margin:20px;"><span style="font-weight:bold;">' + image.attr("fbphotobox-title") + '</span></div>');
            var html =
                '<div class="panel">' +
                '<div class="panel-heading">' +
                '<h3 class="panel-title" style="padding:0">Thông tin chi tiết</h3>' +
                '</div>' +
                '<div class="panel-body">' +
                '<p><strong>Ngày chụp ảnh: </strong></p>' +
                '<p>' + image.attr("fbphotobox-NgayChup") + '</p>' +
                '<p><strong>Ngày đẩy ảnh: </strong></p>' +
                '<p>' + image.attr("fbphotobox-title") + '</p>' +
                '<p><strong>Ghi chú: </strong></p>' +
                '<p>' + image.attr("fbphotobox-note") + '</p>' +
                '<p><strong>Bình luận: </strong></p>' +
                '<p>' + image.attr("fbphotobox-comment") + '</p>' +
                '<p><strong>Người up ảnh: </strong></p>' +
                '<p>' + image.attr("fbphotobox-TenNhanVien") + '</p>' +
                '<p><strong>Số điện thoại người up ảnh: </strong></p>' +
                '<p>' + image.attr("fbphotobox-SoDienThoai") + '</p>'
                ;

            if ((image.attr("fbphotobox-KinhDo") == "0.000000000" && image.attr("fbphotobox-ViDo") == "0.000000000") ||
                (image.attr("fbphotobox-KinhDo").length == 0 && image.attr("fbphotobox-ViDo").length == 0)) {
                html = html +
                    '<p style="color:red"><strong>Chưa có tọa độ</strong></p>' +
                    '</div>' +
                    '</div>';
            }
            else {
                html = html +
                    '<p><a target="_blank" href="http://maps.google.com/maps?q=' + image.attr("fbphotobox-ViDo") + ',' + image.attr("fbphotobox-KinhDo") + '+(My+Point)&z=14&ll="' + image.attr("fbphotobox-ViDo") + ',' + image.attr("fbphotobox-KinhDo") + '>Xem bản đồ vị trí</a></p>';
                    '</div>' +
                    '</div>';
            }
        ...
        refreshBoxSize: function (image) {
            ...
            if (imageWidth < maxWidth) {
                leftContainer.width(imageWidth - 30);
                this.fbpMainImage.css("max-width", imageWidth);
            }
            else {
                leftContainer.width(maxWidth - 30);
            ...
            this.fbpMainContainer.css({
                width: (leftContainer.width() + rightContainer.width() + 30),
            ...
            if (isShow) {
                ...
                if (!this.IsDisplayed) {
                    ...
                    this.photoTransitionAnimation(tmpNode, from, to, function () {
                        ...
                        $(".fbphotobox-main-container").animate({ opacity: 1 }, {
